...
def no_fit(geo, dest_dataset_X_test, dest_dataset_y_test):
    ...
    return evaluate_clf(y_pred, dest_dataset_y_test, None), geo
...
def refit_classifier_all_data(geo, source_dataset_X_train, source_dataset_y_train, source_dataset_X_test, source_dataset_y_test,
    ...
    res_source = evaluate_clf(y_pred_source, source_dataset_y_test, None)
    res_dest = evaluate_clf(y_pred_dest, dest_dataset_y_test, None)
    res_all = evaluate_clf(y_pred_all, np.append(source_dataset_y_test, dest_dataset_y_test), None)
...
if __name__ == "__main__":
    datasets = {
        "sumo_cities_car_vs_bikes": preprocess(r.sumo_cities_car_vs_bikes()),
    ...
    for model_path in tqdm(glob('results/final_models/*.pkl'), position=0, leave=False):
        ...
        for dataset_name, (dest_X_train, dest_X_test, dest_y_train, dest_y_test) \
            ...
            if not os.path.exists(f"results/transfered_models/{model_name}|{dataset_name}_no.pkl"):
                ...
                pd.DataFrame.from_dict([res]).to_csv(f"results/transfered_models/{model_name}|{dataset_name}_no.csv", index=False)
            ...
            if not os.path.exists(f"results/transfered_models/{model_name}|{dataset_name}_partial.pkl"):
                ...
                pd.DataFrame.from_dict([res]).to_csv(f"results/transfered_models/{model_name}|{dataset_name}_partial.csv", index=False)
            ...
            if not os.path.exists(f"results/transfered_models/{model_name}|{dataset_name}_all.pkl"):
                res, new_geo = refit_classifier_all_data(geo, source_X_train, source_y_train, source_X_test, source_y_test,
                                                         dest_X_train, dest_y_train, dest_X_test, dest_y_test)
                pd.DataFrame.from_dict([res]).to_csv(f"results/transfered_models/{model_name}|{dataset_name}_all.csv", index=False)
