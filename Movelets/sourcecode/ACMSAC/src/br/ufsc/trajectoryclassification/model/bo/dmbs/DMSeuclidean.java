...
public class DMSeuclidean implements IDistanceMeasureForSubtrajectory {
	...
	private Pair<ISubtrajectory, Double> getBestAlignmentByPointFeatures(ISubtrajectory s, ITrajectory t) {
		...
		List<IPoint> sub = s.getData();
		List<IPoint> traj = t.getData();

		int diffLength = traj.size() - sub.size();
		...
		for (int i = 0; i <= diffLength; i++) {
			 ...
			 * */
			for (int j = 0; j < sub.size() && currentSum < minSumFound; j++) {

				value = dmbp.getDistance(sub.get(j), traj.get(i + j));
		 ...
		 * */
		double distance = Math.sqrt(minSumFound/sub.size());

		/* It creates and returns the part of the trajectory that
		 * best matches with the subtrajectory.
		 * */
		int start = bestPosition;
		int end = bestPosition + sub.size() - 1;
		return new Pair<>(new Subtrajectory(start, end , t), distance);
	}
	private Pair<ISubtrajectory, Double> getBestAlignmentByPointFeatures_(ISubtrajectory s, ITrajectory t) {
		...
		for (int i = 0; i <= diffLength; i++) {
			...
			if(currentSum == null) {
				...
				double minus = dmbp.getDistance(sub.get(0), traj.get(i));
				double plus = dmbp.getDistance(sub.get(sub.size()-1), traj.get(i + sub.size()-1));
				currentSum = currentSum - minus*minus + plus*plus;
			}
